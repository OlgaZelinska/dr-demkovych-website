...
var QueriesObserver = class extends Subscribable {
	...
	#findMatchingObservers(queries) {
		...
		const getObserver = (options) => {
			const defaultedOptions = this.#client.defaultQueryOptions(options);
			return this.#observers.find((o) => o.options.queryHash === defaultedOptions.queryHash) ?? new QueryObserver(this.#client, defaultedOptions);
		};
...
function dehydrateQuery(query) {
	return {
		...
		...query.state.status === "pending" && { promise: query.promise?.catch((error) => {
...
function dehydrate(client, options = {}) {
	...
	return {
		mutations,
		queries: client.getQueryCache().getAll().flatMap((query) => filterQuery(query) ? [dehydrateQuery(query)] : [])
	};
}
...
//#endregion
//#region node_modules/@tanstack/react-query/build/modern/prefetch.js
function usePrefetchQuery(options) {
	const queryClient = useQueryClient();
	if (!queryClient.getQueryState(options.queryKey)) queryClient.prefetchQuery(options);
}
function usePrefetchInfiniteQuery(options) {
	const queryClient = useQueryClient();
	if (!queryClient.getQueryState(options.queryKey)) queryClient.prefetchInfiniteQuery(options);
}
//#endregion
