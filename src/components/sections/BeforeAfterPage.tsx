...
const BeforeAfterSlider: React.FC<{
  ...
  // Get navigation hint based on position
  const getHint = () => {
    if (allPhotos.length <= 1) return null
    if (photoIndex === 0) return 'Натисніть або свайпніть праворуч'
    if (photoIndex === allPhotos.length - 1) return 'Натисніть або свайпніть ліворуч'
    return 'Натисніть або свайпніть для зміни'
  }

  return (
    <div
      className="relative w-full aspect-[4/5] rounded-2xl overflow-hidden shadow-lg cursor-pointer touch-none group"
      ...
      {hasPhotos ? (
        <>
          ...
          <div className="absolute top-4 left-0 right-0 text-center pointer-events-none">
            <span className="inline-block bg-black/70 backdrop-blur-sm text-white text-xs md:text-sm font-medium px-3 py-1.5 rounded-full shadow-lg animate-pulse">
              {getHint()}
            </span>
          </div>
          ...
          <div className="absolute inset-0 flex items-center justify-between px-4 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <button
              type="button"
              className={`p-2 rounded-full bg-black/50 backdrop-blur-sm text-white hover:bg-black/70 transition-all ${
                photoIndex === allPhotos.length - 1 ? 'opacity-30 cursor-not-allowed' : ''
              }`}
              onClick={(e) => {
                e.stopPropagation()
